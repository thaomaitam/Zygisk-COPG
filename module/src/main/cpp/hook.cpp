...
private:
    ...
    void spoofSystemProperties() {
        ...
        // Fixed property spoofing - corrected ro.product.product to ro.product.name
        if (!deviceConfig.brand.empty()) 
            __system_property_set("ro.product.brand", deviceConfig.brand.c_str());
        if (!deviceConfig.device.empty()) 
            __system_property_set("ro.product.device", deviceConfig.device.c_str());
        if (!deviceConfig.manufacturer.empty()) 
            __system_property_set("ro.product.manufacturer", deviceConfig.manufacturer.c_str());
        if (!deviceConfig.model.empty()) 
            __system_property_set("ro.product.model", deviceConfig.model.c_str());
        if (!deviceConfig.fingerprint.empty()) 
            __system_property_set("ro.build.fingerprint", deviceConfig.fingerprint.c_str());
        
        // CRITICAL FIX: Changed ro.product.product to ro.product.name
        if (!deviceConfig.product.empty()) 
            __system_property_set("ro.product.name", deviceConfig.product.c_str());
        
        // Additional comprehensive property spoofing for better coverage
        if (!deviceConfig.brand.empty()) {
            __system_property_set("ro.product.vendor.brand", deviceConfig.brand.c_str());
            __system_property_set("ro.product.system.brand", deviceConfig.brand.c_str());
        }
        if (!deviceConfig.device.empty()) {
            __system_property_set("ro.product.vendor.device", deviceConfig.device.c_str());
            __system_property_set("ro.product.system.device", deviceConfig.device.c_str());
        }
        if (!deviceConfig.manufacturer.empty()) {
            __system_property_set("ro.product.vendor.manufacturer", deviceConfig.manufacturer.c_str());
            __system_property_set("ro.product.system.manufacturer", deviceConfig.manufacturer.c_str());
        }
        if (!deviceConfig.model.empty()) {
            __system_property_set("ro.product.vendor.model", deviceConfig.model.c_str());
            __system_property_set("ro.product.system.model", deviceConfig.model.c_str());
        }
        if (!deviceConfig.product.empty()) {
            __system_property_set("ro.product.vendor.name", deviceConfig.product.c_str());
            __system_property_set("ro.product.system.name", deviceConfig.product.c_str());
            __system_property_set("ro.build.product", deviceConfig.product.c_str());
        }
        
        LOGD("System property spoofing completed");
    }
};
