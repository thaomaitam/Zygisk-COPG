...
private:
    static constexpr const char* PROPERTY_MAPPINGS[][2] = {
        {"BRAND", "ro.product.brand"},
        {"DEVICE", "ro.product.device"}, 
        {"MANUFACTURER", "ro.product.manufacturer"},
        {"MODEL", "ro.product.model"},
        {"FINGERPRINT", "ro.build.fingerprint"},
        {"PRODUCT", "ro.product.name"},  // Fixed: was ro.product.product
...
public:
    ...
    static void spoofComprehensiveProperties(const DeviceConfig& config) {
        ...
        spoofProperty("ro.product.name", config.product);  // Fixed typo
...
public:
    BuildFieldManager(JNIEnv* environment) : env(environment) {
        ...
        if (!buildClass || !versionClass) {
            ...
            throw std::runtime_error("Build classes not found");
        }
    }
    ...
    void updateAllFields(const DeviceConfig& config) {
        if (config.isEmpty()) {
...
public:
    ...
    void postAppSpecialize(const zygisk::AppSpecializeArgs *args) override {
        try {
            ...
            buildManager.updateAllFields(deviceConfig);

            // Spoof native system properties
            PropertySpoofManager::spoofComprehensiveProperties(deviceConfig);
            
            LOGD("postAppSpecialize => All spoofing operations completed successfully");
        } catch (const std::exception& e) {
            LOGE("postAppSpecialize exception: %s", e.what());
        }
